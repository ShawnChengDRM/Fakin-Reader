    ...
    <div class="book" ref="wrapper" @click="setRead($event)" :style="{color:fontColor,background:background}">
      <div v-if="pagingPattern===0" class="books-con" v-infinite-scroll="loadMore" infinite-scroll-disabled="loading"
        ...
        <div class="book-content" ref="content">
          ...
          <div class="book-inner" v-html="bookChaptersBody(bookContent.content)"
          ...
          <section class="read-section" :style="styleObject">
            <div class="read-section-inner" :style="{columns:clWidth-20+'px',columnGap:10+'px'}"
              ...
              <div class="book-inner" :style="{fontSize:fontSize+'px'}"
                   v-html="bookChaptersBody(bookContent.content)"></div>
            </div>
          </section>
      ...
      <div class="bottom-controll-bar">
        <div class="readSet">
          ...
          <div class="readSet-item pattern">
            <span v-for="(item,index) in pattern" :class="{active:readePattern===index}"
                  @click.stop="setPattern(item,index)">{{item.item}}</span>
          </div>
          <div class="readSet-item">
...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    methods: {
      ...
      //获取阅读章节页面
      _pageIndexCache(){
        return this.getBookLocalStorage()[this.$route.params.id] ? this.getBookLocalStorage()[this.$route.params.id].pageIndexCache : 1
      },
      //书源index
      _bookSourceId(){
      ...
      _bookSource(){
        ...
        return this.getBookLocalStorage()[this.$route.params.id] ? this.getBookLocalStorage()[this.$route.params.id].bookSource : tbook[this.$route.params.id].bookSource
      },
      //书籍信息
      getBook(id){
        book(id).then((res) => {
        ...
        chapterContent(link).then((res) => {
          if (res.data.ok) {
            ...
            if (res.data.chapter.cpContent) {
              this.bookContent = this._nromalBook(res.data.chapter.title, res.data.chapter.cpContent)
            } else {
              this.bookContent = this._nromalBook(res.data.chapter.title, res.data.chapter.body)
            }
            if (this.getBookLocalStorage()[this.$route.params.id]) {
      ...
      getPrevChapter(click){
        chapterContent(this.bookSourceLinks[this.bookCurrentPage - 1]).then((res) => {
          if (res.data.ok) {
            Indicator.close();
            if (res.data.chapter.cpContent) {
              this.prevContent = this._nromalBook(res.data.chapter.title, res.data.chapter.cpContent)
              if (click) {
                this.bookContent = this.prevContent;
              }
            } else {

              this.prevContent = this._nromalBook(res.data.chapter.title, res.data.chapter.body)
              if (click) {
                this.bookContent = this.prevContent
              }
            }
      ...
      getNextChapter(click){
        chapterContent(this.bookSourceLinks[this.bookCurrentPage + 1]).then((res) => {
          if (res.data.ok) {
            Indicator.close();
            if (res.data.chapter.cpContent) {
              this.nextContent = this._nromalBook(res.data.chapter.title, res.data.chapter.cpContent)
              if (click) {
                this.bookContent = this.nextContent
              }
            } else {

              this.nextContent = this._nromalBook(res.data.chapter.title, res.data.chapter.body)
              if (click) {
                this.bookContent = this.nextContent
              }
            }
